...
export default function Admin() {
    ...
    //   const adminUpdateFun = async (id) => {
    //     try {
    //         const resp = await axios.post(`${BaseUrl}/admin/Update/${id}`)
    //         if(resp.status == 200){
    //             getAdminFun()
    //         }
    //     } catch (error) {
                        ...
                        <tbody>
                            {adminlist.length > 0 ? (adminlist.map((item, index) => (
                                <tr>
                                    ...
                                    <td>{item?.email}</td>
                                    <td>
                                    <FaEdit size={25} color='blue' onClick={() => handleShow(item?._id)} />
                                    &nbsp;&nbsp;&nbsp;
                                    <IoTrashBin size={25} color='red' onClick={() => adminDeleteFun(item?._id)} />
                                </td>
                                </tr>
                            ))
